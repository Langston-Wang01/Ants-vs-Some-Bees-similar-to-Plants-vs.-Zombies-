...
class Place:
    ...
    def __init__(self, name: str, exit: Place | None = None):
        ...
        # BEGIN Problem 2
        if self.exit: # if an exit exists 
            self.exit.entrance = self # the entrance of the exit (next place) will be set to the current place
        # END Problem 2
...
class Insect:
    ...
    is_waterproof = False # ALL INSECTS SHOULD NOT BE WATERPROOF UNLESS STATED 
...
class Ant(Insect):
    ...
    def add_to(self, place: Place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            # place.ant represents the ant currently in place
            ...
            if place.ant.is_container and place.ant.can_contain(self):
                ...
                place.ant = self
                place.ant.store_ant(contained_ant)
            else:
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if not self.doubled: # If an ant's damage hasn't been doubled 
            self.damage *= 2 # Double their damage, and indicate that they have been doubled 
            self.doubled = True
        # END Problem 12
...
class HarvesterAnt(Ant):
    ...
    def action(self, gamestate: GameState):
        ...
        """
        # BEGIN Problem 1
        gamestate.food += 1 # increases gamestate food by each turn of harvester ant 
        # END Problem 1
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        current_place = self.place # This is the position of the thrower ant
        distance = 0 # Represents distance between bee and thrower ant instance 

        if len(current_place.bees) >= 1 and (self.lower_bound <= distance <= self.upper_bound):
             # Checks for bees in the ants place (This means it isn't a hive)
            return random_bee(current_place.bees)
        
        next_place = current_place.entrance # Moving towards the Hive to check for bees 

        while next_place: # Continue this loop as long as we don't enter the Hive -> None
            distance += 1
            if len(next_place.bees) >= 1 and not next_place.is_hive and (self.lower_bound <= distance <= self.upper_bound):
                # Should Check if the place contains bees, isn't a hive, and is within the bounds
                return random_bee(next_place.bees)
            next_place = next_place.entrance
        return None # If there are no bees at all 
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount: float):
        ...
        Ant.reduce_health(self, amount) # reduces fire ant instance's health based on amount 

        if self.health <= 0: # If fire ant dies, damage should include death damage
            for bee in current_place_bees:
                bee.reduce_health(self.damage + amount) 
        else: # If the fire ant is alive, reflective damage should only be applied (amount)
            for bee in current_place_bees:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate: GameState):
        if self.cooldown > 0: # Means ant is chewing (counts as its turn)
            self.cooldown -= 1
        else:
            lucky_bee = random_bee(self.place.bees) # Chooses a random bee in its place
            if lucky_bee is not None:
                lucky_bee.reduce_health(lucky_bee.health)
                self.cooldown = HungryAnt.chew_cooldown # We use Class attribute in case of an overide 
# END Problem 7
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other: Ant) -> bool:
        # BEGIN Problem 8a
        # If the "other" ant is not a container AND the container ant is empty 
    ...
    def action(self, gamestate: GameState):
        ...
        if self.ant_contained is not None:
            self.ant_contained.action(gamestate) # Performs the contained ants action
        # END Problem 8a
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate: GameState):
        current_place_bees = list(self.place.bees)
        if self.ant_contained is not None: # If the TankAnt has as ant, perform both its action and the contained ant's
            for bee in current_place_bees:
                bee.reduce_health(self.damage)
            self.ant_contained.action(gamestate)
        else: # If empty, just perform the TankAnt's action
            for bee in current_place_bees:
                bee.reduce_health(self.damage)
# END Problem 9
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate: GameState):
        ...
        """
        # BEGIN Problem 12
        ThrowerAnt.action(self, gamestate) # Performs Queen's action first
        position = self.place.exit # Position behind Queen 
        while position is not None: # As long as we have an avaliable exit
            ant = position.ant
            if ant is not None: # If an ant exists
                if ant.is_container and ant.ant_contained is not None:
                    ant.ant_contained.double() # Should double ants inside container
                position.ant.double() # doubles all ants (including containers (Tank))
            # ** We can double all ants again because once doubled, their doubled attribute becomes True**
    ...
    def reduce_health(self, amount: float):
        ...
        """
        # BEGIN Problem 12
        Insect.reduce_health(self, amount) # Reduces Queen's health
        if self.health <= 0: # If Queen dies, game is over
            return ants_lose()
        # END Problem 12
